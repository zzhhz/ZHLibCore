 ...
 * app前后台切换监听
 */
public class HAppBackgroundListener {
    private static HAppBackgroundListener sInstance;
    ...
    private HAppBackgroundListener() {
    }

    public static HAppBackgroundListener getInstance() {
        if (sInstance == null) {
            synchronized (HAppBackgroundListener.class) {
                if (sInstance == null)
                    sInstance = new HAppBackgroundListener();
            }
        }
        return sInstance;
    }

    public synchronized HAppBackgroundListener init(Application application) {
        if (application == null)
